...
module.exports = function () {
  const importMap = {
    imports: {
      vue: "./core/vue.esm-browser.js",
      "element-plus": "./core/element-plus.full.js",
      typescript: "./core/typescript.js",
      "@vue/devtools-api": "./core/empty-devtools-api.js",
      "vue-router": "./core/vue-router.esm-browser.js",
    },
    scopes: {},
  };
  ...
  const browserFriendlyExtensions = [
    ".esm-browser.js",
    ".esm.browser.js",
    ".browser.esm.js",
    ".esm.js",
  ...
  while (queue.length > 0) {
    ...
    try {
      const modPkgPath = path.join(
        __dirname,
        "../node_modules",
        moduleName,
        "package.json"
      );
      const modPkgContent = fs.readFileSync(modPkgPath, "utf-8");
      const modPkg = JSON.parse(modPkgContent);
      ...
      // 解析 module 字段的目录和文件名
      const moduleRelativePath = moduleEntry.startsWith("./")
        ? moduleEntry.slice(2)
        : moduleEntry;
      ...
      const moduleBaseName = path.basename(
        moduleRelativePath,
        path.extname(moduleRelativePath)
      );
      ...
      try {
        ...
        for (const ext of browserFriendlyExtensions) {
          ...
          if (files.includes(tryFilename)) {
            ...
            resolvedPath = `/node_modules/${moduleName}/${path
              .relative(moduleRootDir, path.join(moduleDir, tryFilename))
              .replace(/\\/g, "/")}`;
            break;
          }
        }
